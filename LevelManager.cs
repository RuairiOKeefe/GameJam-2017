...
public class LevelManager : MonoBehaviour
{
    ...
    private void WaveHandler()
    {
        if (WavesToSpawn > 0)
        {
            if (TimeToWave <= 0)
            {
                NumEnemiesToSpawn = (MaxWaves - WavesToSpawn + 1) * 3;
            }
    ...
    public void SpawnEnemy()
    {
        ...
        Instantiate(EnemyPrefabs[EnemyIndexToSpawn], CandidateTiles[CandidateIndex].transform.position + Vector3.back, Quaternion.identity).GetComponent<Movement>().LevelMap = this.gameObject;
    }
}
